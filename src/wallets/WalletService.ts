...
export class WalletService {
  ...
  connect = async () => {
    if (typeof window.cartesi === "undefined") {
      throw new Error("cartesi is not installed");
    }

    try {
      const account = await window.cartesi.requestAccounts();
      const network = await window.cartesi.requestNetwork();
      this.account = {
        address: account[0],
      };
