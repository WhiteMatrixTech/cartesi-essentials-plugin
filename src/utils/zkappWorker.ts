...
type Transaction = Awaited<ReturnType<typeof cartesi.transaction>>;
...
const functions = {
  setActiveInstanceToBerkeley: async (args: { gqlUrl: string }) => {
    ...
    // const Berkeley = cartesi.Network(args.gqlUrl + "/graphql");
    // cartesi.setActiveInstance(Berkeley);
  },
  ...
  loadContract: async (args: { contract: string; moduleName: string }) => {
    ...
    const ins = cartesi.Network(
      "https://api.minascan.io/node/devnet/v1" + "/graphql"
    );
    cartesi.setActiveInstance(ins);
  ...
  createUpdateTransaction: async (args: {}) => {
    const transaction = await cartesi.transaction(() => {
  ...
  createDeployTransaction: async (args: {
    ...
    const transaction = await cartesi.transaction(feePayerPublickKey, () => {
